...
pub fn run(options: CliOptions) -> Result<()> {
    ...
    let mut colebin = colebin::new(options)?;
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        if let Event::RedrawRequested(_) = event {
            // Draw the colebin
            colebin.draw(pixels.get_frame());

            // Prepare egui
            gui.prepare(&window, &mut colebin.settings);

            // Render everything together
            let render_result = pixels.render_with(|encoder, render_target, context| {
                // Render the colebin texture
        ...
        if input.update(&event) {
            {
                let mut settings = &mut colebin.settings;
